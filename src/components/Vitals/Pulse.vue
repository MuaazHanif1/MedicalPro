...
<template>
  <template v-if="isCreatedVitalView">
    <CreatedVitals heading="Pulse" vitalType="pulse" @show-create-vital="showVitalInput"
      @show-editable-vital="showEditableVitalInput" />
  </template>
  <template v-else>
    <template :key="vital.hash" v-for="(vital, index) in pulse">
      <q-separator v-if="index !== 0" class="width-auto my-30" />
      <PulseInput @set-pulse="setPulseValues" @delete-pulse="removePulseComponent" @create-pulse="createPulse"
        @on-submit="apiCall" :vitalIndex="index" :editableData="editableData" ref="pulseRef" />
    </template>
    <!-- <span
    @click="incrementPulseRate()"
    class="medium-font row flex cursor-pointer items-center mt-30"
  >
    <q-icon class="color-green" name="add_circle"></q-icon>
    <p class="pl-5 color-green ma-0 font-small">Add Pulse Rate</p>
  </span> -->
...
const showEditableVitalInput = (vitalsData) => {
  console.log("pulse data", vitalsData);
...
const pulseRef = ref([]);
const pulse = ref([{ hash: new Date().getTime() }]);


// Will be used in future
/**  Description: this method adds another vital input section and validates the previous input section.
*/
// const incrementPulseRate = () => {
//   // console.log("///incrementPulseRate", skipUnwrap);
//   console.log(pulseRef.value[pulse.value.length - 1]);

//   pulseRef.value[pulse.value.length - 1].validation();
// };
...
*/
const createPulse = () => {
  pulse.value.push({ hash: new Date().getTime() });
};
 ...
 */
const setPulseValues = (values, index) => {
  Object.assign(pulse.value[index], values);
};
 ...
 */
const removePulseComponent = (index) => {
  pulse.value.splice(index, 1);
};
 ...
 */
const onSubmit = () => {
  pulseRef.value[pulse.value.length - 1].validateAndSubmit();
};
...
const apiCall = async () => {
  isSaving.value = true;
  for (const key in pulse.value[0]) {
    if (pulse.value[0][key] === "") {
      Reflect.deleteProperty(pulse.value[0], key);
    }
  }
  Reflect.deleteProperty(pulse.value[0], "hash");

  if (pulse.value[0]['not_performed']) {
    for (const key in pulse.value[0]) {
  ...
  if (pulse.value[0]['not_performed'] === false) {
    Reflect.deleteProperty(pulse.value[0], 'reason');
  }
  const pulseData = pulse.value[0];

  pulseData.appointment_id = store.appointment_id;

  const response = await store.sendPulseVital(pulseData);
  isSaving.value = false;
  if (response.data) {
    if (pulse.value[0].id) {
      editVital(pulse.value[0].id, response.data.data);
    } else {
