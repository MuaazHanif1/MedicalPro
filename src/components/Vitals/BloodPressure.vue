<template>
  <template v-if="isCreatedBloodPressureView">
    <CreatedVitals heading="Blood Pressure" vitalType="bloodPressure" @show-create-vital="showBloodPressureInput"
      @show-editable-vital="showEditableVitalInput" />
  </template>

  <template v-else>
    <template :key="vital.hash" v-for="(vital, index) in bloodPressure">
      <q-separator v-if="index !== 0" class="width-auto my-30" />
      <BloodPressureInput @set-blood-pressure="setBloodPressureValues"
        @delete-blood-pressure="removeBloodPressureComponent" @create-blood-pressure="createBloodPressure"
        @on-submit="apiCall" :vitalIndex="index" :editableData="editableData" ref="bloodPressuresRef" />
    </template>
    <!-- will be used in future -->
    <!-- <span
    @click="incrementBloodPressure()"
    class="medium-font row flex cursor-pointer items-center mt-10"
  >
    <q-icon class="color-green" name="add_circle"></q-icon>
    <p class="pl-5 color-green ma-0 font-small">Add Blood Pressure</p>
  </span> -->
...
import { ref } from "vue";
import BloodPressureInput from "./BloodPressureInput.vue";
...
let isCreatedBloodPressureView = ref(true);


/**  Description: this method loads the input form vital view.
     */
const showBloodPressureInput = () => {
  isCreatedBloodPressureView.value = false;
};
...
const showEditableVitalInput = (vitalsData) => {
  console.log("EDITING BLOOD PRESSURE", vitalsData);
...
const showCreatedVitals = () => {
  ...
  isCreatedBloodPressureView.value = true;
};
const store = useVitalsStore();

// blood pressure vitals
const bloodPressuresRef = ref([]);
const bloodPressure = ref([{ hash: new Date().getTime() }]);

// Will be used in future
/**  Description: this method adds another vital input section and validates the previous input section.
*/
// const incrementBloodPressure = () => {
//   console.log(bloodPressuresRef.value[bloodPressure.value.length - 1]);
//   bloodPressuresRef.value[bloodPressure.value.length - 1].validation();
// };
...
*/
const createBloodPressure = () => {
  bloodPressure.value.push({ hash: new Date().getTime() });
};
 ...
 */
const setBloodPressureValues = (values, index) => {
  console.log("///setting values", values, index);
  Object.assign(bloodPressure.value[index], values);
};
 ...
 */
const removeBloodPressureComponent = (index) => {
  bloodPressure.value.splice(index, 1);
};
 ...
 */
const onSubmit = () => {
  bloodPressuresRef.value[bloodPressure.value.length - 1].validateAndSubmit();
};
...
const addVital = (vital) => {
  store.bloodPressure.unshift(vital);
}
/**  Description: this method edits the vital present in its respective array in the store
 */
const editVital = (id, vital) => {
  //Find index of specific object using findIndex method.    
  let objIndex = store.bloodPressure.findIndex((obj => obj.id == id));
  store.bloodPressure[objIndex] = vital;
}
...
const apiCall = async () => {
  isSaving.value = true;
  for (const key in bloodPressure.value[0]) {
    if (
      bloodPressure.value[0][key] === "" ||
      bloodPressure.value[0][key] === null
    ) {
      console.log(`DELETING ${key} with value `, bloodPressure.value[0].key)
      Reflect.deleteProperty(bloodPressure.value[0], key);
    }
  }
  Reflect.deleteProperty(bloodPressure.value[0], "hash");
  if (bloodPressure.value[0]['not_performed']) {
    for (const key in bloodPressure.value[0]) {
  ...
  if (bloodPressure.value[0]['not_performed'] === false) {
    Reflect.deleteProperty(bloodPressure.value[0], 'reason');
  }
  const bloodPressureData = bloodPressure.value[0];
  bloodPressureData.appointment_id = store.appointment_id;
  console.log("DATA SENDING TO THE APi", bloodPressureData);
  const response = await store.sendBloodPressureVital(bloodPressureData);
  isSaving.value = false;
  if (response.data) {
    if (bloodPressure.value[0].id) {
      editVital(bloodPressure.value[0].id, response.data.data);
    } else {
