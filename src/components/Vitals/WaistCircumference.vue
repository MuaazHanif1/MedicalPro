...
<template>
  <template v-if="isCreatedVitalView">
    <CreatedVitals heading="Waist Circumference" vitalType="waistCircumference" @show-create-vital="showVitalInput"
      @show-editable-vital="showEditableVitalInput" />
  </template>
  <template v-else>
    <div class="relative-position">
      <WaistCircumferenceInput @set-waist-circumference="setWaistValues" @on-submit="apiCall"
        :editableData="editableData" ref="waistRef" />
...
import WaistCircumferenceInput from "./WaistCircumferenceInput.vue";
 ...
 */
const onSubmit = () => {
  waistRef.value.validateAndSubmit();
};
...
const addVital = (vital) => {
  store.waistCircumference.unshift(vital);
}
...
const editVital = (id, vital) => {
  ...
  let objIndex = store.waistCircumference.findIndex((obj => obj.id == id));
  store.waistCircumference[objIndex] = vital;
}
...
const apiCall = async () => {
  isSaving.value = true;
  for (const key in waist.value) {
    if (waist.value[key] === "") {
      Reflect.deleteProperty(waist.value, key);
    }
  }
  const waistCircumferenceData = waist.value;
  delete waistCircumferenceData["measuringUnit"];
  waistCircumferenceData.appointment_id = store.appointment_id;
  const response = await store.sendWaistCircumferenceVital(
  ...
  if (response.data) {
    if (waist.value.id) {
      editVital(waist.value.id, response.data.data);
    } else {
