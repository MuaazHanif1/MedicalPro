<template>
  <template v-if="isCreatedVitalView">
    <CreatedVitals heading="Heart Rate" vitalType="heartRate" @show-create-vital="showVitalInput"
      @show-editable-vital="showEditableVitalInput" />
  </template>
  <template v-else>
    <template :key="vital.hash" v-for="(vital, index) in heartRate">
      <q-separator v-if="index !== 0" class="width-auto my-30" />
      <HeartRateInput @set-heart-rate-pressure="setHeartRateValues"
        @delete-heart-rate-pressure="removeHeartRateComponent" @create-heart-rate-pressure="createHeartRatePressure"
        @on-submit="apiCall" :vitalIndex="index" :editableData="editableData" ref="heartRateRef" />
    </template>
    <!-- will be used in future -->
    <!-- <span
    @click="incrementHeartRate()"
    class="medium-font row flex cursor-pointer items-center mt-10"
  >
    <q-icon class="color-green" name="add_circle"></q-icon>
    <p class="pl-5 color-green ma-0 font-small">Add Heart Rate</p>
  </span> -->
...
<script setup>
import { ref } from "vue";
import HeartRateInput from "./HeartRateInput.vue";
...
const heartRate = ref([{ hash: new Date().getTime() }]);

// const incrementHeartRate = () => {
//   // console.log("///incrementHeartRate", skipUnwrap);
//   console.log(heartRateRef.value[heartRate.value.length - 1]);

//   heartRateRef.value[heartRate.value.length - 1].validation();
// };
...
*/
const createHeartRatePressure = () => {
  heartRate.value.push({ hash: new Date().getTime() });
};
 ...
 */
const setHeartRateValues = (values, index) => {
  Object.assign(heartRate.value[index], values);
};
 ...
 */
const removeHeartRateComponent = (index) => {
  heartRate.value.splice(index, 1);
};
 ...
 */
const onSubmit = () => {
  heartRateRef.value[heartRate.value.length - 1].validateAndSubmit();
};
...
const apiCall = async () => {
  isSaving.value = true;
  for (const key in heartRate.value[0]) {
    console.log("KEY ISSS", key, heartRate.value[0][key]);
    if (heartRate.value[0][key] === "") {
      Reflect.deleteProperty(heartRate.value[0], key);
    }
  }
  Reflect.deleteProperty(heartRate.value[0], "hash");
  if (heartRate.value[0]['not_performed']) {
    for (const key in heartRate.value[0]) {
  ...
  if (heartRate.value[0]['not_performed'] === false) {
    Reflect.deleteProperty(heartRate.value[0], 'reason');
  }

  const heartRateData = heartRate.value[0];

  heartRateData.appointment_id = store.appointment_id;

  const response = await store.sendHeartRateVital(heartRateData);
  isSaving.value = false;
  if (response.data) {
    if (heartRate.value[0].id) {
      editVital(heartRate.value[0].id, response.data.data);
    } else {
