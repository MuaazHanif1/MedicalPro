      ...
      <div class="doctor-calendar pb-20 pr-20 pl-20">
        ...
        <div class="row no-wrap mt-30 mb-20">
          <CalendarIntegration :connected="isGoogleConnected" calendarName="google" class="q-mr-lg"
            @button-click="onButtonClick" :isLoading="isGoogleLoading"  :conncetedEmail="userStore.$state.calendar.googleCalendar && userStore.$state.calendar.googleCalendar.status ? userStore.$state.calendar.googleCalendar.login_email:'' " />
          <CalendarIntegration :connected="isOutlookConnected" calendarName="outlook" @button-click="onButtonClick"
            :isLoading="isOutlookLoading"  :conncetedEmail="userStore.$state.calendar.outlookCalendar && userStore.$state.calendar.outlookCalendar.status ? userStore.$state.calendar.outlookCalendar.login_email:'' "/>
        </div>
      </div>
    </q-card>
  </q-dialog>
...
onMounted(() => {
  ...
  // userPatient.$state.googleCalendarIntegration && (isGoogleConnected.value = userPatient.$state.googleCalendarIntegration.status)
  // userPatient.$state.outlookCalendarIntegration && (isOutlookConnected.value = userPatient.$state.outlookCalendarIntegration.status)
})
...
const setVariableForOutlook = (isConnected) => {
  ...
  axiosApiClient
    ...
    .then(() => {
      isConnected && (isOutlookConnected.value = false) && (userStore.$state.calendar.outlookCalendar = null)
    })
    .catch((error) => {
      console.log(error)
...
const setVariableForGoogle = (isConnected) => {
  ...
  axiosApiClient
    ...
    .then(() => {
      isConnected && (isGoogleConnected.value = false) && (userStore.$state.calendar.googleCalendar = null)
    })
    .catch((error) => {
      console.log(error)
    }).finally(() => {
